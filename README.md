...
OrderBookPerformanceTest.measureArrayOrderBook       avgt    5  3.292 ± 0.950  us/op
OrderBookPerformanceTest.measureIpqOrderBook         avgt    5  1.063 ± 0.021  us/op
OrderBookPerformanceTest.measureLinkedListOrderBook  avgt    5  0.707 ± 0.061  us/op
OrderBookPerformanceTest.measureMapOrderBook         avgt    5  5.202 ± 2.474  us/op
```
Interpretation: testing results are perform on microseconds-per-operation. Where operation is function `orderBookBatchTest` where all methods or `OrderBook` such as `match/add/get/update/remove` are executed in sequential order. And the result is how many microseconds on average this batch of operations on OrderBook (around 10 operations total - read javadocs comments for the method) takes. From here you can see for example that array-based order book outperforms map-based.
