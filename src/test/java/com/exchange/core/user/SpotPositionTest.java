...
public class SpotPositionTest {
  ...
  public void exceptionTest() {
    ...
    Assertions.assertEquals(lock.getMessage(),
        "Failed to lock more than available: amount=200, balance=0");

    AppException unlock = Assertions.assertThrows(AppException.class,
        () -> position.unlock(b200), "Exception should be thrown");
    Assertions.assertEquals(unlock.getMessage(),
        "Failed to unlock more than locked: amount=200, locked=0");

    AppException freeLock = Assertions.assertThrows(AppException.class,
        () -> position.freeLocked(b200), "Exception should be thrown");
    Assertions.assertEquals(freeLock.getMessage(),
        "Failed to free more than locked: amount=200, locked=0");
  }
}